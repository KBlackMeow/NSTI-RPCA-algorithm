...
%     stopCriterion = norm(Z, 'fro') / d_norm;
    stopCriterion = (norm(E_hat, 1)+ norm(Z, 'fro'))/ d_norm;
    time(iter) = toc(t1);
    err(iter)=stopCriterion;
    fprintf('Iter no. %d err %e time %f \n', iter, err(iter), time(iter));

    if (err(iter) <= max(tol,eps))
        converged = 1;
        fprintf('Target error reached.\n');
    end
    if (iter >10 && (err(iter-10)/ err(iter) -1) < 0.0001)
       converged = 1;
       fprintf('No improvement.\n');
    end    
    
    if mod( total_svd, 10) == 0 && 0
